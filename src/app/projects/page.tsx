...
import { Project } from '@/types/project'; // types/project.ts に全フィールドが定義されている前提
import { 
  MagnifyingGlassIcon, 
...
type EditableProject = Omit<Project, 'id' | 'created_at'>;
...
export default function ProjectsPage() {
  ...
  const [editFormData, setEditFormData] = useState<Partial<EditableProject>>({});
  // -------------------------
  ...
  const initialFilters: Filters = {
    ...
    transaction_type: '',
    product_category: '',
    installation_scheduled_date_from: '',
    installation_scheduled_date_to: '',
    removal_scheduled_date_from: '',
    removal_scheduled_date_to: '',
  };
  const [filters, setFilters] = useState<Filters>(initialFilters);

  // --- フィルターの選択肢 ---
  const statusOptions = ['成約', '作業完了', '発送済', '設置済', '撤去済'];
  const salesPersonOptions = ['西井', '佐藤', '鈴木', '高橋']; // 本来はマスタテーブルから取得するのが望ましい
  const transactionTypeOptions = ['レンタル', '販売'];
  ...
  const fetchProjects = async (currentFilters: Filters = filters) => {
    setLoading(true);
  ...
  const handleEditClick = (project: Project) => {
    ...
    Object.keys(formattedProject).forEach(key => {
      if (key.includes('date') && formattedProject[key as keyof Project]) {
        try {
          (formattedProject as any)[key] = new Date(formattedProject[key as keyof Project] as string).toISOString().split('T')[0];
        } catch(e) { /* no-op */ }
      }
    });
  ...
  const handleSaveClick = async (id: number) => {
    ...
    for (const key in dataToUpdate) {
      if (key.includes('date') && dataToUpdate[key as keyof Project] === '') {
  ...
  const totalColumns = 41; // 表示する列の総数
                ...
                projects.map((project) => (
                  <tr key={project.id} className="hover:bg-gray-50">
                    {editingId === project.id ? (
                      // --- 編集モード ---
                      <>
                        <td className="px-4 py-2 text-sm font-medium sticky left-0 bg-white"><div className='flex gap-2'><button onClick={() => handleSaveClick(project.id)} className="text-green-600 hover:text-green-900"><CheckIcon className='h-5 w-5'/></button><button onClick={handleCancelClick} className="text-gray-500 hover:text-gray-800"><XMarkIcon className='h-5 w-5'/></button></div></td>
                        ...
                        <td className="px-1 py-1"><input type="text" name="product_specs" value={editFormData.product_specs || ''} onChange={handleEditFormChange} className="w-48 text-sm border-gray-300 rounded-md" /></td>
                        {Array.from({ length: 10 }).map((_, i) => (<td key={i} className="px-1 py-1"><input type="text" name={`accessory_${i + 1}`} value={editFormData[`accessory_${i + 1}` as keyof Project] as string || ''} onChange={handleEditFormChange} className="w-40 text-sm border-gray-300 rounded-md" /></td>))}
                        ...
                        <td className="px-1 py-1"><input type="date" name="installation_request_submission_date" value={editFormData.installation_request_submission_date || ''} onChange={handleEditFormChange} className="w-40 text-sm border-gray-300 rounded-md" /></td>
                        ...
                        <td className="px-1 py-1"><input type="date" name="removal_request_submission_date" value={editFormData.removal_request_submission_date || ''} onChange={handleEditFormChange} className="w-40 text-sm border-gray-300 rounded-md" /></td>
                        ...
                        <td className="px-1 py-1"><input type="date" name="post_removal_inspection_date" value={editFormData.post_removal_inspection_date || ''} onChange={handleEditFormChange} className="w-40 text-sm border-gray-300 rounded-md" /></td>
                      ...
                      <>
                        ...
                        <td className="px-4 py-4 text-sm text-gray-500">{project.product_specs}</td>
                        ...
                        <td className="px-4 py-4 text-sm text-gray-500">{project.installation_request_submission_date}</td>
                        ...
                        <td className="px-4 py-4 text-sm text-gray-500">{project.removal_request_submission_date}</td>
                        ...
                        <td className="px-4 py-4 text-sm text-gray-500">{project.post_removal_inspection_date}</td>
                        <td className="px-4 py-4 text-sm text-gray-500">{project.warranty_end_date}</td>
